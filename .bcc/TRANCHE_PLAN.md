...
| T8 | Governed Work Loop Prototype | **IMPLEMENTED, gate 52/52, NOT PARKED** (0037, 0038) — blocked on its own stop condition "the discovery pass is clean": the harness's `front_door`, `tool_health` and `enforcement` axes have not executed since 2026-08-18, and three certifications reported PASS anyway. Not a T8 defect; factory work. — awareness → impact → `read_file` → preview → diff → witness-bound approval → Apply → measured changed paths → `test`/`lint` verification → refresh. Opens with **three safety preconditions**: `patch`'s absent write declaration, governance failing open on Apply, and uninterpretable tool output reported as success |
...
| **The harness attaches by overriding `SUITE_PROJECT_ROOT`** — `_harness/ro_probe_inner.py:32` sets it in the child env, and T6's instance binding correctly refuses it. `front_door`, `tool_health` (12 → 0 probes) and `enforcement` are all computed from that probe, so one root cause takes the discovery pass down. Dated between the 2026-08-18 07:20 and 10:33 harness runs | 0038 | **HIGH — blocks T8 park and the acceptance walk.** Factory work, not product: the guard is right and the harness attaches the way T6 forbade. Fix is to BIND the probe instance to its target rather than override transport |
