...
class SessionPanel(tk.LabelFrame):
    """Owns session CRUD widgets and signal subscriptions."""

    def __init__(self, parent, backend, colors, root):
        super().__init__(
            parent,
            text=" Session ",
        ...
        self.root = root
        self._build()
        self.backend.bus.subscribe("session_switched", self._on_session_switched)

    def _build(self):
        c = self.colors
        inner = tk.Frame(self, bg=c.get('background'))
        inner.pack(fill='x', padx=8, pady=6)

        # Row 1: dropdown + Switch
        row1 = tk.Frame(inner, bg=c.get('background'))
        row1.pack(fill='x')

        self.session_var = tk.StringVar()
        self.session_dropdown = ttk.Combobox(
            row1, textvariable=self.session_var, state='readonly', font=("Segoe UI", 9)
        )
        self.session_dropdown.pack(side='left', fill='x', expand=True)
        self._refresh_dropdown()

        _bb = {"relief": "flat", "font": ("Segoe UI", 9)}
        tk.Button(
            row1, text="Switch",
            bg=c.get('panel_bg'), fg=c.get('button_fg', c.get('foreground')),
            command=self._on_switch, **_bb
        ).pack(side='left', padx=(4, 0))

        # Row 2: stats
        self.stats_var = tk.StringVar(value="")
        tk.Label(inner, textvariable=self.stats_var,
                 bg=c.get('background'), fg=c.get('accent'),
                 font=("Segoe UI", 8, "italic")).pack(anchor='w', pady=(3, 0))

        # Row 3: action buttons
        row3 = tk.Frame(inner, bg=c.get('background'))
        row3.pack(fill='x', pady=(4, 0))

        tk.Button(row3, text="+ New",
                  bg=c.get('panel_bg'), fg=c.get('button_fg', c.get('foreground')),
                  command=self._on_new, **_bb).pack(side='left')
        tk.Button(row3, text="Save",
                  bg=c.get('panel_bg'), fg=c.get('button_fg', c.get('foreground')),
                  command=self._on_save, **_bb).pack(side='left', padx=(4, 0))
        tk.Button(row3, text="Delete",
                  bg=c.get('error', c.get('accent')), fg=c.get('button_fg', 'white'),
                  command=self._on_delete, **_bb).pack(side='right')
    ...
    def refresh_theme(self, new_colors):
        ...
        c = new_colors
        self.configure(bg=c.get('background'), fg=c.get('foreground'))
        for child in self.winfo_children():
            try:
                child.configure(bg=c.get('background'))
            except Exception:
                pass
            for sub in (child.winfo_children() if hasattr(child, 'winfo_children') else []):
                try:
                    sub.configure(bg=c.get('background'), fg=c.get('foreground'))
                except Exception:
                    pass
    ...
    def _refresh_dropdown(self):
        ...
        self.session_dropdown['values'] = names
        current = self.backend.current_session.name
        self.session_var.set(current if current in names else (names[0] if names else ""))

    def _refresh_stats(self):
        s = self.backend.current_session
        self.stats_var.set(f"Session: {s.name}  |  Chunks: {s.chunk_count}")

    def _on_session_switched(self, data):
        self.root.after(0, self._refresh_dropdown)
        self.root.after(0, self._refresh_stats)

    def _on_switch(self):
        name = self.session_var.get()
        if name and name != self.backend.current_session.name:
            self.backend.switch_session(name)
    ...
    def _on_new(self):
        ...
        if not name or not name.strip():
            return
        try:
            self.backend.new_session(name.strip())
        except ValueError as e:
            messagebox.showerror("Error", str(e), parent=self.root)

    def _on_save(self):
        self.backend.save_current_session()
        messagebox.showinfo("Saved", f"Session '{self.backend.current_session.name}' saved.", parent=self.root)

    def _on_delete(self):
        name = self.backend.current_session.name
        if not messagebox.askyesno("Delete Session", f"Delete session '{name}'? This cannot be undone.", parent=self.root):
            return
        self.backend.delete_session(name)


class IngestPanel(tk.LabelFrame):
    """Owns file/folder ingest widgets and signal subscriptions."""

    DEFAULT_EXTENSIONS = ['.py', '.js', '.ts', '.md', '.txt', '.json', '.yaml', '.yml', '.toml', '.sh', '.html', '.css']
    ...
    def __init__(self, parent, backend, colors, root):
        ...
        self.root = root
        self._build()
        self.backend.bus.subscribe("session_stats_updated", self._on_stats_updated)
        self.backend.bus.subscribe("session_switched", self._on_session_switched)
    ...
    def _build(self):
        ...
        row1 = tk.Frame(inner, bg=c.get('background'))
        row1.pack(fill='x')
        tk.Label(row1, text="Embed model:", bg=c.get('background'), fg=c.get('foreground'),
                 font=("Segoe UI", 8)).pack(side='left')
        self.embed_var = tk.StringVar(value=self.backend.config_store.get('embed_model') or 'nomic-embed-text')
        embed_models = ['nomic-embed-text', 'mxbai-embed-large', 'all-minilm']
        self.embed_combo = ttk.Combobox(row1, textvariable=self.embed_var, values=embed_models,
                                         font=("Segoe UI", 9), width=18)
        self.embed_combo.pack(side='left', padx=(4, 0))

        # Row 2: ingest buttons
        row2 = tk.Frame(inner, bg=c.get('background'))
        row2.pack(fill='x', pady=(4, 0))
        _bb = {"relief": "flat", "font": ("Segoe UI", 9)}
        tk.Button(row2, text="+ File",
                  bg=c.get('panel_bg'), fg=c.get('button_fg', c.get('foreground')),
                  command=self._on_ingest_file, **_bb).pack(side='left')
        tk.Button(row2, text="+ Folder",
                  bg=c.get('panel_bg'), fg=c.get('button_fg', c.get('foreground')),
                  command=self._on_ingest_folder, **_bb).pack(side='left', padx=(4, 0))
        tk.Button(row2, text="Clear Vectors",
                  bg=c.get('error', c.get('accent')), fg=c.get('button_fg', 'white'),
                  command=self._on_clear, **_bb).pack(side='right')

        # Ingested files listbox
        self.files_listbox = tk.Listbox(
            inner, height=3,
            bg=c.get('entry_bg', c.get('panel_bg')),
            fg=c.get('entry_fg', c.get('foreground')),
            font=("Consolas", 8), selectmode='browse', relief='flat'
        )
        self.files_listbox.pack(fill='x', pady=(4, 0))

        # Chunk count
        self.chunk_var = tk.StringVar(value="Chunks: 0")
        tk.Label(inner, textvariable=self.chunk_var,
                 bg=c.get('background'), fg=c.get('accent'),
                 font=("Segoe UI", 8, "italic")).pack(anchor='w', pady=(3, 0))

        self._refresh_display(self.backend.current_session)
    ...
    def _refresh_display(self, session):
        self.chunk_var.set(f"Chunks: {session.chunk_count}")
        self.files_listbox.delete(0, 'end')
        for f in session.ingested_files:
            self.files_listbox.insert('end', f)

    def _on_stats_updated(self, session):
        self.root.after(0, lambda: self._refresh_display(session))

    def _on_session_switched(self, data):
        self.root.after(0, lambda: self._refresh_display(data))

    def _on_ingest_file(self):
        path = filedialog.askopenfilename(parent=self.root, title="Select file to ingest")
        if not path:
            return
        embed_model = self.embed_var.get()
        self.chunk_var.set("Ingesting...")
        threading.Thread(
            target=self.backend.ingest_file,
            args=(path, embed_model),
            daemon=True
        ).start()

    def _on_ingest_folder(self):
        path = filedialog.askdirectory(parent=self.root, title="Select folder to ingest")
        if not path:
            return
        embed_model = self.embed_var.get()
        self.chunk_var.set("Ingesting...")
        threading.Thread(
            target=self.backend.ingest_directory,
            args=(path, self.DEFAULT_EXTENSIONS, embed_model),
            daemon=True
        ).start()

    def _on_clear(self):
        if not messagebox.askyesno("Clear Vectors", "Remove all ingested vectors from this session?", parent=self.root):
            return
        self.backend.clear_session_vectors()
...
class CELL_UI:
    def __init__(self, shell, backend):
        ...
        # Task Queue panel widgets
        self.panel_taskqueue = None
        self.task_listbox = None
        self.btn_add_task = None
        self.btn_remove_task = None
        self.btn_run_all = None
        self.queue_progress_lbl = None

        # RAG / Session panels
        self.session_panel = None
        self.ingest_panel = None
    ...
    def _register_signals(self):
        ...
        if hasattr(self.backend, 'bus'):
            ...
            self.backend.bus.subscribe("queue_progress", self._on_queue_progress)
            self.backend.bus.subscribe("queue_complete", self._on_queue_complete)
    ...
    def _setup_main_window(self):
        ...
        # ------------------------------------------------------------------
        # SESSION + INGEST panels (right column, top)
        # ------------------------------------------------------------------
        self.session_panel = SessionPanel(self.container, self.backend, self.colors, self.shell.root)
        self.session_panel.pack(in_=self.right_col, fill='x', padx=10, pady=(10, 6))

        self.ingest_panel = IngestPanel(self.container, self.backend, self.colors, self.shell.root)
        self.ingest_panel.pack(in_=self.right_col, fill='x', padx=10, pady=(0, 8))
        ...
        self.infer_log.insert('1.0', "Inference stream will appear here when a task runs.\n")
        ...
        self.result_text.insert('1.0', "Model response will appear here after a run.\n")
        ...
        # PANEL 4 — Task Queue
        self.panel_taskqueue = tk.LabelFrame(
            self.container,
            text=" Task Queue ",
            fg=self.colors.get('foreground'),
            bg=self.colors.get('background'),
        ...
        self.panel_taskqueue.pack(in_=self.right_col, fill='x', padx=10, pady=(0, 12))

        _tq = tk.Frame(self.panel_taskqueue, bg=self.colors.get('background'))
        _tq.pack(fill='x', padx=10, pady=8)
        ...
        self.task_listbox = tk.Listbox(
            ...
            bg=self.colors.get('entry_bg', self.colors.get('panel_bg')),
            fg=self.colors.get('entry_fg', self.colors.get('foreground')),
            selectbackground=self.colors.get('accent'),
            selectforeground=self.colors.get('button_fg', 'white'),
            font=("Segoe UI", 9),
            activestyle='none',
            relief='flat'
        )
        self.task_listbox.pack(fill='x', pady=(0, 6))
        self.task_listbox.bind('<Double-Button-1>', lambda e: self._load_task_into_ui())

        # Button row
        _btn_row = tk.Frame(_tq, bg=self.colors.get('background'))
        _btn_row.pack(fill='x', pady=(0, 4))

        _qbtn = {"relief": "flat", "font": ("Segoe UI", 9)}
        self.btn_add_task = tk.Button(
            _btn_row, text="+ Add Task",
            bg=self.colors.get('panel_bg'),
            fg=self.colors.get('button_fg', self.colors.get('foreground')),
            command=self._add_task_from_ui, **_qbtn
        )
        self.btn_add_task.pack(side='left', padx=(0, 4))

        self.btn_remove_task = tk.Button(
            _btn_row, text="✕ Remove",
            bg=self.colors.get('panel_bg'),
            fg=self.colors.get('button_fg', self.colors.get('foreground')),
            command=self._remove_selected_task, **_qbtn
        )
        self.btn_remove_task.pack(side='left', padx=(0, 4))

        self.btn_clear_queue = tk.Button(
            _btn_row, text="Clear All",
            bg=self.colors.get('panel_bg'),
            fg=self.colors.get('button_fg', self.colors.get('foreground')),
            command=self._clear_queue, **_qbtn
        )
        self.btn_clear_queue.pack(side='left')

        self.btn_run_all = tk.Button(
            _btn_row, text="▶ Run All",
            bg=self.colors.get('accent'),
            fg=self.colors.get('button_fg', 'white'),
            command=self._on_run_all, **_qbtn
        )
        self.btn_run_all.pack(side='right')

        self.btn_load_workflow = tk.Button(
            _btn_row, text="Load Workflow",
            bg=self.colors.get('panel_bg'),
            fg=self.colors.get('button_fg', self.colors.get('foreground')),
            command=self._on_load_workflow, **_qbtn
        )
        self.btn_load_workflow.pack(side='left', padx=(4, 0))

        # Progress label
        self.queue_progress_lbl = tk.Label(
            _tq, text="",
            bg=self.colors.get('background'),
            fg=self.colors.get('accent'),
            font=("Segoe UI", 8, "italic")
        )
        self.queue_progress_lbl.pack(fill='x')
    ...
    def _restore_component_state(self):
        ...
        # Restore task queue display from persisted session
    ...
    def refresh_theme(self, new_colors=None):
        ...
        if self.session_panel is not None:
            self.session_panel.refresh_theme(self.colors)
        if self.ingest_panel is not None:
            self.ingest_panel.refresh_theme(self.colors)
        ...
        if hasattr(self, 'btn_load_workflow') and self.btn_load_workflow is not None:
            self.btn_load_workflow.configure(
                bg=self.colors.get('panel_bg'),
                fg=self.colors.get('button_fg', self.colors.get('foreground'))
            )
    ...
    def _add_task_from_ui(self):
        """Captures current UI config as a new task and appends it to the queue."""
        content = self.input_box.get("1.0", "end-1c").strip()
        if not content:
            messagebox.showwarning("Empty Task", "Please enter task content before adding.", parent=self.shell.root)
            return

        model = self.model_var.get()
        role = self.role_entry.get()
        prompt = self.prompt_text.get("1.0", "end-1c")
        label = content.splitlines()[0][:50]
        ...
        # Clear input box so user can set up the next task
        self.input_box.delete("1.0", "end")
        self.input_box.focus_set()
    ...
    def _remove_selected_task(self):
        ...
        if not selected:
            return
        self.backend.remove_task(selected[0])
        self._refresh_task_listbox()
    ...
    def _clear_queue(self):
        ...
        self._refresh_task_listbox()
        if self.queue_progress_lbl:
            self.queue_progress_lbl.configure(text="")

    def _load_task_into_ui(self):
        """Double-click: loads selected task back into UI fields for editing."""
        selected = self.task_listbox.curselection()
        if not selected:
            return
        task = self.backend.task_queue[selected[0]]
        self._update_widget(self.role_entry, task.get("role", ""))
        self._update_widget(self.prompt_text, task.get("system_prompt", ""))
        self._update_widget(self.input_box, task.get("user_content", ""))
        if task.get("model") in self.model_dropdown['values']:
            self.model_var.set(task["model"])

    def _refresh_task_listbox(self):
        """Redraws the task listbox from backend.task_queue."""
        if not self.task_listbox:
            return
        self.task_listbox.delete(0, 'end')
        for i, task in enumerate(self.backend.task_queue):
            self.task_listbox.insert('end', f"{i + 1}. {task.get('label', f'Task {i + 1}')}")

    def _on_run_all(self):
        """Kicks off sequential execution of all queued tasks."""
        if not self.backend.task_queue:
            messagebox.showwarning("Empty Queue", "No tasks in queue. Add tasks first.", parent=self.shell.root)
            return
        if self.btn_run_all:
            self.btn_run_all.configure(state='disabled')
        if self.queue_progress_lbl:
            self.queue_progress_lbl.configure(text="Starting...")
        self.backend.run_queue()

    def _on_queue_progress(self, data):
        """Updates the progress label and highlights the active task."""
        ...
        def update():
            if self.queue_progress_lbl:
                self.queue_progress_lbl.configure(text=f"Running {current}/{total}: {label}")
            if self.task_listbox:
                self.task_listbox.selection_clear(0, 'end')
                self.task_listbox.selection_set(current - 1)
                self.task_listbox.see(current - 1)
            # Clear inference log for new task
            if self.infer_log:
                self.infer_log.configure(state='normal')
                self.infer_log.delete('1.0', 'end')
                self.infer_log.insert('1.0', f"[Queue] Task {current}/{total}: {label}\n")
                self.infer_log.configure(state='disabled')

        self.shell.root.after(0, update)

    def _on_load_workflow(self):
        """Load a workflow JSON file and append its tasks to the queue."""
        path = filedialog.askopenfilename(
            parent=self.shell.root,
            initialdir='_workflows',
            title="Load Workflow",
        ...
        try:
            ...
            messagebox.showerror("Load Error", f"Could not read workflow:\n{e}", parent=self.shell.root)
            return
        tasks = wf.get('tasks', [])
        for task in tasks:
            self.backend.add_task(task)
        ...
        messagebox.showinfo("Workflow Loaded", f"Loaded '{name}' — {len(tasks)} task(s) added.", parent=self.shell.root)

    def _on_queue_complete(self, data):
        """Re-enables Run All and shows completion status."""
        def update():
            if self.queue_progress_lbl:
                self.queue_progress_lbl.configure(text=f"Done — {len(self.backend.task_queue)} tasks complete.")
            if self.btn_run_all:
