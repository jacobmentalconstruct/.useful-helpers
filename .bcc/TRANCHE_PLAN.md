...
| **`secret_audit` cannot scan this whole repository** — it exceeds `DEFAULT_TIMEOUT_S = 120` and the seam kills it. Narrowed to `src/` it completes with 0 findings over 27 files | post-parity audit, 2026-08-20 | Low-medium. This is Charter §7.4's recorded module-level-timeout frailty showing up in practice rather than in theory: the timeout is not a per-call parameter, so a large tree cannot be scanned at all. Either make the bound a parameter or have the tool page its own work |
| **`dead_code` reports 8 high-confidence false positives when scoped to `src/`** — `invoke.cancel`, `watch.cursor`/`poll`, `payload.materialise`, `event_log.count`/`record_decision` are all called from `gates/` | post-parity audit, 2026-08-20 | Informational, not a defect. The analyser answers honestly *for the root it was given*; the callers are simply outside it. Worth stating because a future reader running the same command will see the same eight and may believe them. `cartridge_conflicts` remains the one genuine entry |
| **`domain_boundary_audit` reports 38 crossings and cannot judge them** — `verdict: "none (no policy supplied)"` | 0034, re-confirmed 2026-08-20 | Low. Unchanged: without a declared layering policy the tool reports neutral facts. Already recorded below |
